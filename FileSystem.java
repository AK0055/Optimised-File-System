...
import java.util.LinkedList;//Files
import java.util.Queue;//File alloc table
import java.util.TreeMap;//Date,time
//max FDT size=544KB
//max FAT size=480KB
//size per entry in FDT=42bytes

class FileSystem
{	//max app size=20MB
	static int totalFileEntry=13250,//max no. of files
	clustersize=128,//block size
	totalfat=163840;//max no. of blocks
    ...
    static String getName( int in ) throws Exception //gets name of file from address
    {
    	...
    	for( int i=0 ; i<10 ; i++ ) 
    	{
    		if( a.charAt(i)==(char)0 ) break;//waiting till end of string
	...
	public static void main(String[] args) throws Exception
    {
    	...
    	file = new RandomAccessFile("disk.txt","rw");//create disk.txt to store info of files created
    	for( int i=0 ; i<totalFileEntry ; i++ )
    	{
    		...
    		file.read(b);
    		if( (char)(b[0])==(char)0 )
    			pqfE.add(i+1);	//add file index to queue
    		else
    		{
    			String name = getName(i+1); 	
    			index.put(name, i+1);	//fetch name to treemap
    		}
    	}
    	...
    	{
    		System.out.print("\n>>>   ");
    		String[] s = br.readLine().split(" "); //split command string into substrings of command and file name
    		if( s[0].equals("createf") )
    		{
			if( s.length < 2 )// if file name not given
    			{
    			...
    			char p=permi();//init permission set function
    			int is = createFile(s[1], p);
    			if( is==1 ) //return value from create function
			...
			else if( index.containsKey(s[1]) ) //if file already in treemap, read
    			{
    		...
    		else if( s[0].equals("writef") )
    		{
				...
				else if( index.containsKey(s[2]) )//if file already has content, append
	    			{
